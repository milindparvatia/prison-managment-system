...
if(isset($_GET['Update']))
{
	...
	$res1=mysqli_query($conn,"select * from permission WHERE ACCESS IS NULL LIMIT 1");
				...
				if(isset($_GET['radio'])){
					...
					$gender=$_GET['radio'];
						...
						return $result;
							...
							$sql = "DROP TRIGGER IF EXISTS `grant`;";
							$res = ExecuteSetQuery($sql);

							$sql = "CREATE TRIGGER `grant` AFTER UPDATE ON `permission`
							FOR EACH ROW
							BEGIN
							IF EXISTS (SELECT * FROM permission WHERE `UserID`=$_SESSION[UserID] AND `ACCESS`=$_GET[radio] LIMIT 1)
	...
	$sql = $conn->query("UPDATE permission SET ACCESS = '{$gender}' where UserID=$_SESSION[UserID]");
		
		phpAlert(   "You Have Successfully Granted a Permission"   );	
...
<div id="Navbar">
  ...
  <tbody>
    ...
    <?php
	 $res=mysqli_query($conn,"select * from permission WHERE ACCESS IS NULL LIMIT 1");
    ...
    <tr class="StyleTextField">
      ...
      <td>
        ...
        <p>
         <?php echo $row['ACCESS']; ?>
          <label>
            <input type="radio" name="radio" value="1" <?php if($row['ACCESS']=="1") {echo "checked";}?> id="radio">
            yes</label>
          
          <label>
            <input type="radio" name="radio" value="0" <?php if($row['ACCESS']=="0") {echo "checked";}?> id="radio">
            no</label>
          </p>
        <p><br> 
        </p></td>
